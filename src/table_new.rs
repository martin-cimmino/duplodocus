...
use crate::bitpack_vec::{BitPackedVec, BitPackedSlice, BitPackedSliceMut, BitPackedSliceIter};
...
impl<'s, 't> SuffixTable<'s, 't> {
    ...
    pub fn into_parts(self) -> (Cow<'s, [u8]>, Cow<'t, BitPackedVec>) {
    ...
    #[inline]
    pub fn table(&self) -> &BitPackedVec {
        &self.table
    }
    ...
    pub fn suffix(&self, i: usize) -> &[u8] {
        &self.text[self.table.get(i as usize)..]
    }
...
impl<'s, 't> fmt::Debug for SuffixTable<'s, 't> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for (rank, sufstart) in self.table.as_slice().iter().enumerate() {
            writeln!(
                f,
                "suffix[{}] {}",
                rank,
...
#[allow(dead_code)]
fn naive_table(text: &[u8]) -> BitPackedVec {
    ...
    let mut bpvec = BitPackedVec::new(text.len(), text.len());
    for (i, el) in table.into_iter().enumerate() {
    	bpvec.set(i, el as usize);
    };
    bpvec
    
}

fn sais_table<'s>(text: &'s [u8]) -> BitPackedVec {
    assert!(text.len() <= u64::MAX as usize);

    let mut sa = BitPackedVec::new(text.len(), text.len());
    let mut sa_slice = sa.as_mut_slice();
    let mut stypes = SuffixTypes::new(text.len() as u64);
    let mut bins = Bins::new();
    sais(&mut sa_slice, &mut stypes, &mut bins, &Utf8(text));
    
    sa
}

fn sais<T>(sa: &mut BitPackedSliceMut, stypes: &mut SuffixTypes, bins: &mut Bins, text: &T)
where
    T: Text,
    <<T as Text>::IdxChars as Iterator>::Item: IdxChar,
{
    ...
    */
    ...
    for i in 0..sa.len() {
        let sufi = sa.get(i as usize);
        if sufi > 0 {
            let (lasti, lastc) = text.prev(sufi.try_into().unwrap());
            if stypes.is_desc(lasti) {
                bins.head_insert(sa, lasti, lastc);
            }
        }
    ...
    for i in 0..sa.len() {
        let sufi = sa.get(i as usize);
        if stypes.is_valley(sufi.try_into().unwrap()) {
            sa.set(num_wstrs as usize, sufi);
            num_wstrs += 1;
        }
    }
    ...
    for i in num_wstrs..(sa.len() as u64) {
        sa.set(i as usize, sa.max_val)
    }
    for i in 0..num_wstrs {
        let cur_sufi = sa.get(i as usize) as u64;
        if prev_sufi == 0 || !text.wstring_equal(stypes, cur_sufi.try_into().unwrap(), prev_sufi) {
            name += 1;
            prev_sufi = cur_sufi;
        }
        ...
        sa.set((num_wstrs + (cur_sufi / 2)) as usize, name as usize - 1);
    }
    ...
    for i in (num_wstrs..(sa.len() as u64)).rev() {
        if sa.get(i as usize) != sa.max_val {
            sa.set(j as usize, sa.get(i as usize));
            j -= 1;
        }
    }
    ...
    if name < num_wstrs {
        ...
        let (mut r_sa, r_text) = sa.split_at_mut(split_at);

        
        let r_sa = r_sa.slice_to(num_wstrs as usize);


        //sais(&mut r_sa, stypes, bins, &LexNames(&r_text));
        stypes.compute(text);
    } else {
        for i in 0..num_wstrs {
            let reducedi = sa.get(((sa.len() as u64) - num_wstrs + i) as usize);
            sa.set(reducedi as usize, i.try_into().unwrap());
        }
    }
    ...
    for i in 0..num_wstrs {
        let sufi = sa.get(i as usize);
        sa.set(i as usize, sa.get((sa.len() as u64 - num_wstrs as u64 + sufi as u64) as usize));
    }
    ...
    for i in (0..num_wstrs).rev() {
        let sufi = sa.get(i as usize);
        sa.set(i as usize, 0);
        bins.tail_insert(sa, sufi.try_into().unwrap(), text.char_at(sufi.try_into().unwrap()));
    }
...
impl Bins {
    ...
    #[inline]
    fn head_insert(&mut self, sa: &mut BitPackedSliceMut, i: u64, c: u64) {
        let ptr = &mut self.ptrs[c as usize];
        sa.set(*ptr as usize, i.try_into().unwrap());
        *ptr += 1;
    }

    #[inline]
    fn tail_insert(&mut self, sa: &mut BitPackedSliceMut, i: u64, c: u64) {
        let ptr = &mut self.ptrs[c as usize];
        sa.set(*ptr as usize, i.try_into().unwrap());
        if *ptr > 0 {
            *ptr -= 1;
...
struct LexNames<'s>(&'s BitPackedSlice<'s>);
...
impl<'s> Text for LexNames<'s> {
    ...
    #[inline]
    fn prev(&self, i: u64) -> (u64, u64) {
        (i - 1, self.0.get(i as usize - 1).try_into().unwrap())
    }

    #[inline]
    fn char_at(&self, i: u64) -> u64 {
        self.0.get(i as usize).try_into().unwrap()
    }

    fn char_indices(&self) -> std::iter::Enumerate<std::slice::Iter<'s, u64>> {
        self.0.iter().enumerate()
    }

    fn wstring_equal(&self, stypes: &SuffixTypes, w1: u64, w2: u64) -> bool {
        let w1chars = self.0.slice_from(w1 as usize).iter().enumerate();
        let w2chars = self.0.slice_from(w2 as usize).iter().enumerate();
        for ((i1, c1), (i2, c2)) in w1chars.zip(w2chars) {
            let (i1, i2) = (w1 + i1 as u64, w2 + i2 as u64);
            if c1 != c2 || !stypes.equal(i1, i2) {
                return false;
