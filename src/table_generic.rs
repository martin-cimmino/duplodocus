...
impl<'s, 't, I: CompactUint> SuffixTableGeneric<'s, 't, I> {
    ...
    /// list of suffixes. Each suffix is represented by a XXX bit integer and
    /// is a **byte index** into `text`.
    ///
    /// # Panics
    ///
    /// Panics if the `text` contains more than `2^XXX - 1` bytes. This
    ...
    /// (~4GB).
    pub fn new<S>(text: S) -> Result<Self, &'static str>
    where
        S: Into<Cow<'s, [u8]>>,
    {   
        let text = text.into();
        if text.len() as u64 > I::max_value() {
            return Err("Text too large for index type");
        }
        let mut sys = System::new_all();
        sys.refresh_memory();        
        let table = Cow::Owned(sais_table_generic::<I>(&text));
        let mut sys = System::new_all();
        sys.refresh_memory();        
        Ok(SuffixTableGeneric {
            text: text,
            table: table,
    ...
    #[doc(hidden)]
    #[allow(dead_code)]
    pub fn new_naive<S>(text: S) -> SuffixTableGeneric<'s, 't>
    where
        S: Into<Cow<'s, [u8]>>,
    {
        let text = text.into();
    ...
    #[allow(dead_code)]
    pub fn from_parts<S, T>(text: S, table: T) -> SuffixTableGeneric<'s, 't>
    where
        S: Into<Cow<'s, [u8]>>,
        T: Into<Cow<'t, [u64]>>,
    {
    ...
    pub fn into_parts(self) -> (Cow<'s, [u8]>, Cow<'t, [I]>) {
    ...
    pub fn lcp_lens(&self) -> Vec<u64> {
        ...
        for (rank, &sufstart) in self.table().iter().enumerate() {
            inverse[sufstart.to_u64() as usize] = rank as u64;
        }
    ...
    pub fn suffix(&self, i: usize) -> &[u8] {
        &self.text[self.table[i].to_u64() as usize..]
    }
    ...
    ///
    /// ```rust
    /// use suffix::SuffixTableGeneric;
    ///
    /// let sa = SuffixTableGeneric::new("The quick brown fox.");
        ...
        query.len() > 0
            && self
                ...
                .binary_search_by(|&sufi| {
                    self.text[sufi.to_u64() as usize..]
                        .iter()
                        .take(query.len())
                        .cmp(query.iter())
                })
    ...
    /// use suffix::SuffixTableGeneric;
    ///
    /// let sa = SuffixTableGeneric::new("The quick brown fox was very quick.");
    /// assert_eq!(sa.positions("quick"), &[4, 29]);
    /// ```
    #[allow(dead_code)]
    pub fn positions(&self, query: &[u8]) -> &[I] {
        ...
        let start = binary_search(&self.table, |&sufi| query <= &self.text[sufi.to_u64() as usize..]);
        let end = start
            + binary_search(&self.table[start..], |&sufi| {
                !self.text[sufi.to_u64() as usize..].starts_with(query)
            });
...
impl<'s, 't> fmt::Debug for SuffixTableGeneric<'s, 't> {
...
#[allow(dead_code)]
fn lcp_lens_quadratic<I: CompactUint>(text: &[u8], table: &[I]) -> Vec<u64> {
    ...
    for (i, win) in table.windows(2).enumerate() {
        lcps[i + 1] = lcp_len(&text[win[0].to_u64() as usize..], &text[win[1].to_u64() as usize..]);
    }
    lcps
}
...
#[allow(dead_code)]
fn naive_table<I: CompactUint>(text: &[u8]) -> Vec<I> {
    assert!(text.len() as u64 <= I::max_value());
    let mut table = vec![I::zero(); text.len()];
    for i in 0..table.len() {
        table[i] = I::from_u64(i as u64);
    }
    table.sort_by(|&a, &b| text[a.to_u64() as usize..].cmp(&text[b.to_u64() as usize..]));
    table
}

fn sais_table_generic<'s, I: CompactUint>(text: &'s [u8]) -> Vec<I> {
    assert!(text.len() as u64 <= I::max_value());
...
fn sais<T, I>(sa: &mut [I], stypes: &mut SuffixTypes, bins: &mut Bins, text: &T)
where
    T: Text,
    I: CompactUint,
    <<T as Text>::IdxChars as Iterator>::Item: IdxChar,
{
    ...
    for i in 0..sa.len() {
        ...
        if stypes.is_valley(sufi) {
            sa[num_wstrs as usize] = I::from_u64(sufi);
            num_wstrs += 1;
        }
    }
    ...
    for i in 0..num_wstrs {
        ...
        sa[(num_wstrs + (cur_sufi / 2)) as usize] = I::from_u64(name - 1);
    }
    ...
    if name < num_wstrs {
        ...
        for i in 0..num_wstrs {
            let reducedi = I::to_u64(sa[((sa.len() as u64) - num_wstrs + i) as usize]);
            sa[reducedi as usize] = I::from_u64(i);
        }
    }
...
impl Bins {
    ...
    #[inline]
    fn head_insert<T: CompactUint>(&mut self, sa: &mut [T], i: u64, c: u64) {
        let ptr = &mut self.ptrs[c as usize];
        sa[*ptr as usize] = T::from_u64(i);
        *ptr += 1;
    }

    #[inline]
    fn tail_insert<T: CompactUint>(&mut self, sa: &mut [T], i: u64, c: u64) {
        let ptr = &mut self.ptrs[c as usize];
        sa[*ptr as usize] = T::from_u64(i);
        if *ptr > 0 {
            *ptr -= 1;
...
struct LexNames<'s, I>(&'s [I]);

impl<'s, I: CompactUint> Text for LexNames<'s, I> {
    type IdxChars = iter::Enumerate<slice::Iter<'s, I>>;
    ...
    #[inline]
    fn prev(&self, i: u64) -> (u64, u64) {
        (i - 1, self.0[i as usize - 1].to_u64())
    }

    #[inline]
    fn char_at(&self, i: u64) -> u64 {
        self.0[i as usize].to_u64()
    }

    fn char_indices(&self) -> iter::Enumerate<slice::Iter<'s, I>> {
        self.0.iter().enumerate()
    }
...
impl<'a, I: CompactUint> IdxChar for (usize, &'a I) {
    #[inline]
    fn idx_char(self) -> (usize, u64) {
        (self.0, self.1.to_u64())
    }
}
